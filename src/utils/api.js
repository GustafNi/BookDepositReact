...
const requestApiKey = async () => {
  const cachedKey = localStorage.getItem('https://www.forverkliga.se/JavaScript/api/crud.php?requestKey')

  if (cachedKey) {
    return cachedKey
  } else {
    try {
      const apiKey = await fetch(`${'https://www.forverkliga.se/JavaScript/api/crud.php?requestKey'}?requestKey`)
